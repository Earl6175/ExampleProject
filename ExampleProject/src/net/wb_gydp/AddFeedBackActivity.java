 ...
 * 我要反馈
 * */
public class AddFeedBackActivity extends BaseActivity implements AMapLocationListener{
	...
	private Handler handler = new Handler(); // mlp add
	private String pid;
...
//		XtomProcessDialog.show(mContext, "正在获取您的位置信息");
	...
	private void sendFeedBack(){
		...
		if(isNull(value)&&infor == null){
			XtomToastUtil.showShortToast(mContext, "反馈的内容和图片至少有一项目\n不能为空，请重新填写");
			return; 
		}else{
			if(!isNull(value)){
		...
		RequestInformation info = RequestInformation.ADD_FEEDBACK;
	...
	protected void callBackForGetDataSuccess(XtomNetTask netTask, Object result) {
		...
		case TaskConstant.UPLOAD_FILE_SIMPLE:
			...
			break;
		case TaskConstant.ADD_FEEDBACK:
			...
			case ServiceConstant.STATUS_SUCCESS:
				...
				Intent it = new Intent(mContext, FeedBackListActivity.class);
				it.putExtra("pid", pid);
				startActivity(it);
				finish();
				break;
