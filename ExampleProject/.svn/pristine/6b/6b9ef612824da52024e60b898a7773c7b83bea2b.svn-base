 ...
 * 收藏的项目的数据库操作
 * */
public class Favorite_ProjectList_DBClient extends DBHelper {

	private static Favorite_ProjectList_DBClient mClient;

	public Favorite_ProjectList_DBClient(Context context) {
		super(context);
	}

	public static Favorite_ProjectList_DBClient get(Context context) {
		return mClient == null ? mClient = new Favorite_ProjectList_DBClient(
				context) : mClient;
	}
	...
	public boolean isExist(ProjectListInfor infor,String uid) {
		SQLiteDatabase db = getWritableDatabase();
		String sql = ("select * from " + FAVORITE_PROJECTLIST
	...
	public boolean delete(ProjectListInfor infor, String uid) {
		...
		SQLiteDatabase db = getWritableDatabase();
		try {
			db.execSQL("delete from " + FAVORITE_PROJECTLIST
					+ " where pid ='" + infor.getPid() + "'& uid = '"+uid +"'");
		} catch (SQLException e) {
			success = false;
		}
	...
	public void clear(String uid) {
		SQLiteDatabase db = getWritableDatabase();
		db.execSQL("delete from " + FAVORITE_PROJECTLIST +" where uid = '"+uid+"'");
		db.close();
	}
	...
	public boolean isEmpty(String uid) {
		SQLiteDatabase db = getWritableDatabase();
		Cursor cursor = db.rawQuery("select * from " + FAVORITE_PROJECTLIST +" where uid = '"+uid+"'",
				null);
	...
	public ArrayList<ProjectListInfor> selectAll(String uid) {
		...
		String sql = "select pid, title, donation_num, praise_num, comment_num, percent, description, focus_img, focus_img_large from "
				+ FAVORITE_PROJECTLIST + " where uid = '" + uid + "'";
		SQLiteDatabase db = getWritableDatabase();
		Cursor cursor = db.rawQuery(sql, null);
