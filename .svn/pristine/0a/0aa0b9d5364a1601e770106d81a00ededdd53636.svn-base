 ...
 * 发布的项目的数据库操作
 * */
public class My_ProjectList_DBClient extends DBHelper {

	private static My_ProjectList_DBClient mClient;

	public My_ProjectList_DBClient(Context context) {
		super(context);
	}

	public static My_ProjectList_DBClient get(Context context) {
		return mClient == null ? mClient = new My_ProjectList_DBClient(
				context) : mClient;
	}
	...
	public boolean isExist(ProjectListInfor infor,String uid) {
		SQLiteDatabase db = getWritableDatabase();
		String sql = ("select * from " + MY_PROJECTLIST
	...
	public boolean delete(ProjectListInfor infor, String uid) {
		...
		SQLiteDatabase db = getWritableDatabase();
		try {
			db.execSQL("delete from " + MY_PROJECTLIST
					+ " where pid ='" + infor.getPid() + "'& uid = '"+uid +"'");
		} catch (SQLException e) {
			success = false;
		}
	...
	public void clear(String uid) {
		SQLiteDatabase db = getWritableDatabase();
		db.execSQL("delete from " + MY_PROJECTLIST +" where uid = '"+uid+"'");
		db.close();
	}
	...
	public boolean isEmpty(String uid) {
		SQLiteDatabase db = getWritableDatabase();
		Cursor cursor = db.rawQuery("select * from " + MY_PROJECTLIST +" where uid = '"+uid+"'",
				null);
