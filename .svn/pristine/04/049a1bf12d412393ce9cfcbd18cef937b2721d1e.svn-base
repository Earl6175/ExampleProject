...
import net.wb_gydp.db.Favorite_ProjectList_DBClient;
...
public class MyFavoriteListActivity extends BaseActivity{
	...
	private Favorite_ProjectList_DBClient mClient;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		setContentView(R.layout.activity_donation);
		super.onCreate(savedInstanceState);
		mClient = Favorite_ProjectList_DBClient.get(mContext);
		if(!mClient.isEmpty(uid)){
			getDataFromCache();
		}else{
			if(hasNetWork())
				getFavoriteList(current_page,"刷新");
			else{
	...
	private void getFavoriteList(int current_page, String description) {
		HashMap<String, String> params = new HashMap<String, String>();
		params.put("uid", uid);
		params.put("page", String.valueOf(current_page));
		
		RequestInformation information = RequestInformation.GET_MY_FAVORITE_LIST;
	...
	protected void setListener() {
		title.setText(R.string.project_shoucang);
		...
		layout.setOnStartListener(new OnStartListener() {
			...
			public void onStartLoadmore(XtomRefreshLoadmoreLayout v) {
				current_page++;
				getFavoriteList(current_page, "加载");
			}
		});
	}
	...
	protected void callBeforeDataBack(int taskID) {
		switch (taskID) {
		case TaskConstant.GET_MY_FAVORITE_LIST:
			break;
		}
	}
