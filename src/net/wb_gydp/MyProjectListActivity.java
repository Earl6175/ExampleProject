...
import net.wb_gydp.db.My_ProjectList_DBClient;
...
public class MyProjectListActivity extends BaseActivity{
	...
	private My_ProjectList_DBClient mClient;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		setContentView(R.layout.activity_fabu);
		super.onCreate(savedInstanceState);
		mClient = My_ProjectList_DBClient.get(mContext);
		if(!mClient.isEmpty(uid)){
			getDataFromCache();
		}else{
			if(hasNetWork())
				getProjectList(current_page,"刷新");
			else{
	...
	private void getProjectList(int current_page, String description) {
		HashMap<String, String> params = new HashMap<String, String>();
		params.put("uid", uid);
		params.put("page", String.valueOf(current_page));
		
		RequestInformation information = RequestInformation.GET_MY_PRO_LIST;
	...
	protected void setListener() {
		title.setText(R.string.project_fabu);
		...
		layout.setOnStartListener(new OnStartListener() {
			...
			public void onStartLoadmore(XtomRefreshLoadmoreLayout v) {
				current_page++;
				getProjectList(current_page, "加载");
			}
		});
	}
	...
	protected void callBeforeDataBack(int taskID) {
		switch (taskID) {
		case TaskConstant.GET_MY_PRO_LIST:
			break;
		}
	}
